...
function vincularUsuario(id, idDispositivo) {
  const query = `INSERT INTO usuario_maquina (fk_usuario, fk_dispositivo, data_hora, ativo) VALUES ('${id}', '${idDispositivo}', GETDATE(), 1);`;
   return database.executar(query);
}
...
function atualizarDispositivo(idFuncionario, hostName){
   const query = `INSERT INTO usuario_maquina ( fk_usuario, fk_dispositivo, data_hora, ativo) VALUES (${idFuncionario}, (select id from dispositivo where host_name = '${hostName}'), GETDATE(), 1)`;
   return database.executar(query);
}
